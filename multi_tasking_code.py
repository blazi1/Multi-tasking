...
expyriment.stimuli.TextScreen("", heading_size = 40, text = "The experiment consists of two parts, each with three blocks. The first part is starting now.").present()
...
exp.data_variable_names = ["Block", "Trial", "Sex", "Age", "Key", "RT", "Response", "Task Switch or not", "Congruent or not"]
...
def determine_correct(current_stimulus):
	...
	correct_determined = ""
	congruent = False
	if current_stimulus[0:2] == "s3" or current_stimulus[0:2] == "d2":  #congruent
		congruent = True
	...
	return correct_determined, congruent
...
right_or_wrong = ""
previous_stimulus = "  x"
congruent_or_not = True
task_switch = False
for block in exp.blocks:
    for trial in block.trials:
        number_of_correct_responses = 0
        first, second = greet(counter_trials)
        if block == exp.blocks[1]: #somewhat changed text in the second block
        ...
        for stimulus in trial.stimuli:
            ...
            if key == 1073741904:
                ...
                pressed_key = "RIGHT" 
            if block == exp.blocks[0]: #training block #!!!!!  0
                current_stimulus = ordering[0][counter_trials][counter_stimuli]
                correct_answer, congruent_or_not = determine_correct(current_stimulus)
            elif block == exp.blocks[1]: #real block  #!!!!!!!! 1
            ...
            if block == exp.blocks[1]: #in block 2 we add data 
                exp.data.add([trial.id+1, counter_stimuli+1, sex, age, pressed_key, rt, right_or_wrong, task_switch, congruent_or_not])
            counter_stimuli += 1
        counter_stimuli = 0
        counter_trials += 1
    counter_trials = 0
